...
def audit():
    if request.method == "POST":
        ...
        if selected_ids:
            # Combined report (synchronous for testing)
            history = load_history()
            selected_entries = [h for h in history if h["id"] in selected_ids]
            combined_results = "\n\n".join(
                [f"ID {h['id']}: {h['result'][:1000]}" for h in selected_entries]
            )  # Limit each result to 1000 chars
            if len(combined_results) > 5000:
                combined_results = combined_results[:5000] + "... (truncated)"
            ...
            if escenario or file_content:
                ...
                system_prompt = "Eres un experto en auditorías de ciberseguridad. Siempre ejecuta escaneos en tiempo real usando las herramientas disponibles, no uses información histórica o previa. Debes llamar a las funciones de herramientas para cada análisis requerido. Usa las funciones de herramientas proporcionadas para analizar la URL. Responde de forma estructurada: Hallazgos, Evidencias, Mitigaciones."
...
def history():
    ...
    if start_date:
        ...
        history_data = [
            ...
            if datetime.fromisoformat(h["timestamp"]) >= start_dt
        ]
    if end_date:
        end_dt = datetime.fromisoformat(end_date)
        history_data = [
            h for h in history_data if datetime.fromisoformat(h["timestamp"]) <= end_dt
        ]
    ...
    if keyword:
        history_data = [
            h
            for h in history_data
            if keyword in h.get("prompt", "").lower()
            or keyword in h.get("result", "").lower()
        ]
...
def settings():
    if request.method == "POST":
        ...
        settings = {}
        if provider == "Ollama":
            settings["ollama_url"] = request.form.get(
                "ollama_url", "http://localhost:11434/v1"
            )
            settings["ollama_model"] = request.form.get(
                "ollama_model", "llama3.2:latest"
            )
        elif provider == "OpenAI":
            settings["openai_key"] = request.form.get("openai_key")
            settings["openai_url"] = request.form.get(
    ...
    return render_template("settings.html", models=get_ollama_models())
